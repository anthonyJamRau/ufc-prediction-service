...
#     if 'fight_id' not in ufc_fight_stats.columns:
    ufc_fight_stats['fight_id'] = ufc_fight_stats['url'].map(fight_url_dict)
...
def save_to_file(ufc_events,ufc_fights,ufc_fight_stats,ufc_fighters):
    ...
    fight_stat_columns = ['fight_stat_id', 'fight_id', 'fighter_id', 'knockdowns', 'total_strikes_att',
           ...
           'ctrl_time','head_strikes_att','head_strikes_succ','body_strikes_att',
           'body_strikes_succ','leg_strikes_att','leg_strikes_succ','distance_strikes_att','distance_strikes_succ',
           'clinch_strikes_att','clinch_strikes_succ','ground_strikes_att','ground_strikes_succ','url']
...
def normalise_tables():
    ...
    path = os.getcwd() + '/scraped_files'
    ufc_events = pd.read_csv(path + '/' +'ufc_event_data.csv', encoding='ISO-8859-1')
    ufc_fights = pd.read_csv(path + '/' + 'ufc_fight_data.csv', encoding='ISO-8859-1')
    ufc_fight_stats = pd.read_csv(path + '/' + 'ufc_fight_stat_data.csv', encoding='ISO-8859-1')
    ufc_fighters = pd.read_csv(path + '/' + 'ufc_fighter_data.csv', encoding='ISO-8859-1')
